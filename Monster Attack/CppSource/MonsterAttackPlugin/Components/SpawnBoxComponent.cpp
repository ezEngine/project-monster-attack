...
void ezSpawnBoxComponent::StartSpawning()
{
  ...
  {
    // spawn one object right away
    // this is meant for low spawn counts
    // so that when spawning only 2 objects over 1 second, you get the first spawn immediately,
    // then 1 second wait, then a second spawn at the end
    // without this, you would get no spawn for half a second, then 1, then half a second wait, then the second
    Spawn(1);
...
void ezSpawnBoxComponent::OnTriggered(ezMsgComponentInternalTrigger& msg)
{
  ...
  const auto uiTargetSpawnCount = ezMath::Clamp<ezUInt16>(static_cast<ezUInt16>(((tActive.GetSeconds() / m_SpawnDuration.GetSeconds()) * m_uiTotalToSpawn)) + 1, 0, m_uiTotalToSpawn);
