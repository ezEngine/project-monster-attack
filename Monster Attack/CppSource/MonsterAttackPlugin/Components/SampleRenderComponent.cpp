...
#include <MonsterAttackPlugin/Components/SampleRenderComponent.h>
...
EZ_BEGIN_COMPONENT_TYPE(SampleRenderComponent, 1 /* version */, ezComponentMode::Static)
{
  ...
  EZ_BEGIN_ATTRIBUTES
  {
    new ezCategoryAttribute("MonsterAttack"), // Component menu group
  }
  EZ_END_ATTRIBUTES;
...
void SampleRenderComponent::Update()
{
  ...
  {
    const ezBoundingBox bbox = ezBoundingBox::MakeFromCenterAndHalfExtents(ezVec3::MakeZero(), ezVec3(m_fSize));
  ...
  {
    const ezBoundingSphere sphere = ezBoundingSphere::MakeFromCenterAndRadius(ezVec3::MakeZero(), m_fSize);
